...
class CurrencyController extends Controller
{
	...
	*/
	...
	'actions'=>array('admin','delete','create','update','view','updatecurrency'),
	'roles'=>array('ADM'),
	'users'=>array('@'),
	),
	...
	*/
	...
	if($model->save())
		{
						Yii::app()->user->setFlash('success', "Data $model->name Saved");
	...
	*/
	...
	if($model->save())
	{
						Yii::app()->user->setFlash('success', "Data $model->name Updated");
	...
	{
	$model->attributes=$_POST['Currency'];
	$model->change_rate_updated=$now;
	$model->change_rate_updated_by=Yii::app()->user->id;
	$model->ip_user_updated=$_SERVER['REMOTE_ADDR'];
	if($model->save())
	{
						$history= new CurrencyChangeHistory;
						$history->id_currency=$model->id_currency;
						$history->change_rate=$model->change_rate;
						$history->SK=$model->SK;
						$history->created_date=$model->change_rate_updated;
						$history->created_user=$model->change_rate_updated_by;
	...
	if(Yii::app()->request->getIsAjaxRequest())
			 {
	          echo $this->renderPartial('updatecurrency',array('model'=>$model),true,true);//This will bring out the view along with its script.
	          }
	...
	*/
	...
	if(!isset($_GET['ajax']))
	Yii::app()->user->setFlash('success', " Data With Primary Key  '$model->id_currency' has deleted.");

...
class AccountCoaController extends Controller
{
	...
	*/
	public function accessRules()
	{
		return array(
			...
			'actions'=>array('admin','delete','updatelevel'),
			'users'=>array('admin'),
			),
	...
	*/
	...
	{
	$model=new AccountCoa;

	// Uncomment the following line if AJAX validation is needed
	// $this->performAjaxValidation($model);

	if(isset($_POST['AccountCoa']))
	{
	$model->attributes=$_POST['AccountCoa'];
	if($model->save()){
	Yii::app()->user->setFlash('success', " Data Saved");
	$this->redirect(array('admin','id'=>$model->id_account_coa));
	}
	}
	if(Yii::app()->request->getIsAjaxRequest())
	{
	...
	*/
	...
	{
	$dataProvider=new CActiveDataProvider('AccountCoa');
	...
	*/
	...
	{
	$model=new AccountCoa('search');
	$model->unsetAttributes();  // clear any default values
	if(isset($_GET['AccountCoa']))
	$model->attributes=$_GET['AccountCoa'];
	...
	public function actionUpdatelevel()
	  {
			...
			$criteria->params = array(':level'=>$_POST['hasil_level']);

	  
				  
		 echo CHtml::dropDownList('AccountCoa[id_account_coa_parent]','',
		 CHtml::listData(AccountCoa::model()->findAll($criteria), 'id_account_coa', 'account_name'),
	...
	*/
	...
	{
	$model=AccountCoa::model()->findByPk($id);
	if($model===null)
	...
	*/
	...
	{
	if(isset($_POST['ajax']) && $_POST['ajax']==='account-coa-form')
	{

...
class spalquoController extends Controller
{
	...
	public function actionQuo()
	{
		...
		$this->render('../spalquo/admin',array(
		'model'=>$model,
		));
	}
	...
	public function actionQuoupdate($id)
	{
		...
		$this->render('../spalquo/update',array(
		'model'=>$model,
		));
	}
	...
	public function actionQuocreate()
	{
		...
		if(isset($_POST['Quotation']))
		{
			$model->attributes=$_POST['Quotation'];
			 ...
			 // di hilangkan dulu blok nya 
			if ($_POST['Quotation']['id_bussiness_type_order']=='300'){ // diblock  
			Yii::app()->user->setFlash('success', "Maaf Untuk Tipe Time Charter Tidak Bisa Dibuatkan SPAL");
			$this->redirect(array('spal/admin'));
			}
		...
		$this->render('../spalquo/create',array(
		'model'=>$model,
		));
	}
	...
	public function actionQuotationview($id)
	{
	...
	{	
							...
							if ($model->IsSingleRoute==0){
								...
								Yii::app()->user->setFlash('success', " Data Updated");
									 if($_GET['sostat']=='create'){
										  $this->redirect(array('spal/create','id'=>$model->id_quotation));
									 }else{
										   $this->redirect(array('spal/update','id'=>$_GET['soid']));
									 }
								}else{
		...
		if(Yii::app()->request->getIsAjaxRequest())
		{
				  echo $this->renderPartial('../spalquo/_form2_on_so',array('model'=>$model),true,true);//This will bring out the view along with its script.
				  }

		else 
		$this->render('../spalquo/create2',array(
		'model'=>$model,
		));
	}
	...
	$this->render('../spalquo/create2TC',array(
	'model'=>$model,
	));
	}
	...
	$this->render('../spalquo/view',array(
	'model'=>$this->loadModel($id,'Quotation'),
	));
	}

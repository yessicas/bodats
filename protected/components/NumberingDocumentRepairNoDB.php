<?php
class NumberingDocumentRepairNoDB {
	
	
	public static function getFormatNumber($model,$attributePk,$attributeNo,$attributeMonth,$attributeYear,$valuepk=0){
		
	$criteria = new CDbCriteria();
	$criteria->condition = $attributePk.'<>:'.$attributePk.' AND repair_no<>:repair_no'; 
	$criteria->params = array(':'.$attributePk=>$valuepk,':repair_no'=>'');
    ...
    if($query){
	    ...
	    $yearnow=NumberingDocumentRepairNoDB::getYearNow();
	    $monthnow=NumberingDocumentRepairNoDB::getMonthNow();
		...
		$DocumentCode= NumberingDocumentRepairNoDB::getDocumentCode();
	    $RomawiMonth= NumberingDocumentRepairNoDB::getRomawiMonth($monthnow);

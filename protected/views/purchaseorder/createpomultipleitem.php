...
<?php
	$no=0;
	foreach($cid as $idprdet=>$val){
		$no++;
		//echo $idprdet.'<br>';
		
		echo Chtml::hiddenField('cid['.$idprdet.']', $val,array('value'=>$val,'readonly'=>'readonly')); ;
		$SECOND_PART = '';
		
		$detailsNumber = 0;
			$detail = PurchaseRequestDetail::model()->findByPk($idprdet);
			$model = PurchaseRequest::model()->findByAttributes(array('id_purchase_request'=>$detail->id_purchase_request));

				//$RES .= $no."). ".PurchaseRequestDB::getItemDetailPR($detail)." - ".$detail->quantity." ".$detail->metric."\r\n";
				$SECOND_PART .= '
					<td>'.PurchaseRequestDB::getItemDetailPR($detail).' '.PurchaseRequestDB::getDedicatedTo($detail->PurchaseRequest).'</td>
					<td>'.$detail->quantity." ".$detail->metric.'</td>
				';
				$id_item = $detail->id_item;
				
				$SECOND_PART .= displayRowTextInput($idprdet, $detail->id_purchase_request_detail, $id_item, $detail->quantity, $detail->metric, 0, 0, "IDR", "");
		...
		<tr>
		<td '.$rowspan.'>'.$no.'. </td>
		<td '.$rowspan.'>'.$detail->PurchaseRequest->PRNumber.'</td>
		<td '.$rowspan.'>'.$detail->PoCategory->category_name.'</td>';
...
?>
	...
	<tr>
		...
		<td><?php echo $form->textField($modelpo,'ppn',array('class'=>'span12','maxlength'=>2)); ?></td>
		<td></td>
	</tr>
	<tr>
		<td colspan="7" style="text-align:right;">PBBKB</td>
		<td><?php echo $form->textField($modelpo,'pbbkb',array('class'=>'span12','maxlength'=>2)); ?></td>
		<td></td>
	</tr>
...
<div class="view">
	...
	if($modelpo->isNewRecord){
		...
		//echo $form->hiddenField($modelpo,'POYear',array('class'=>'span4','maxlength'=>32,'value'=> NumberingDocumentDBPurchaseOrder::getYearNow(),'readonly'=>'readonly')); 
		 ...
		 //echo $form->textFieldRow($modelpo,'PONumber',array('class'=>'span5','maxlength'=>32,'readonly'=>'readonly')); // ini koq muncul ya
	}
	?>

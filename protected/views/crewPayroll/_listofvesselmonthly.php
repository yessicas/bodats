...
<?php
		...
		echo '<h2>Crew of '.$vessel->VesselName.' '.Yii::app()->dateFormatter->format("MMMM",strtotime(date("Y-".$month."-d"))).' '.$year.'</h2>';
?>
...
<tr>
	...
	<th  align="center" bgcolor="#1A354F" > Pay date </th>
	<th  align="center" bgcolor="#1A354F" > Pay Tipe </th>
...
<?php
	...
	foreach($LISTPOSITION as $position){
		...
		if(isset($crewas[$position->id_crew_position])){
			...
			$LISTCREWPAYROLL = PayrollDB::getListCrewPayrollmonthly($cr->CrewId,$month,$year); // monthly
			$LISTAMOUNT = array();
			$currency='';
			$pay_date='';
			$pay_tipe='';
			foreach($LISTCREWPAYROLL as $amount){
				...
				$pay_date=$amount->transfer_date;
				$pay_tipe=$amount->transfer_type;
			}
			$url = Yii::app()->createUrl("crewPayroll/addpayrollmonthly", array('CrewId'=>$cr->CrewId, 'id_tug'=>$id_tug,'month'=>$month,'year'=>$year));
			...
			echo '<td style="text-align:right">'.$pay_date.'</td>';
			echo '<td style="text-align:right">'.$pay_tipe.'</td>';
			echo '

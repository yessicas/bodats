...
<h3 align="center"> FAKTUR PAJAK </h3>
	...
	<tr class="even">
		...
		<td style="border-bottom: 1px solid #fff !important ;" colspan ='4' id="no_border">
		: <?php echo $modelvoyage->Customer->CompanyName; ?>
		</td>
	</tr>
	...
	<tr class="even">
		...
		<td style="border-bottom: 1px solid #fff !important ;" colspan ='4' id="no_border">
		: <?php echo $modelvoyage->Customer->Address ?> <?php echo $modelvoyage->Customer->Address2 ?>
		</td>
	</tr>
	<tr class="even">
		<td colspan ='2'  class="no_border" id="no_border">
		...
		: <?php echo $modelvoyage->Customer->NPWP ?>
		</td>
	</tr>
	...
	<tr class="even" id="report_content">
		...
		<?php if($modelvoyage->id_bussiness_type_order!=250){ ?>
		...
		<?php
		if($modelvoyage->id_bussiness_type_order==250){
		 	echo NumberAndCurrency::formatCurrency($modelvoyage->QuantityPrice).' x '.NumberAndCurrency::formatNumber($modelvoyage->TotalQuantity).' MT ' ;
		 }else{
		 	echo NumberAndCurrency::formatCurrency($modelvoyage->TCPrice).' x '.NumberAndCurrency::formatNumber($modelvoyage->TotalQuantity).' Unit';
		 }
		?>   

		<?php //echo Yii::app()->numberFormatter->formatCurrency($modelvoyage->Price,"") ?>   <?php //echo  NumberAndCurrency::formatNumber($modelvoyage->Capacity) ?> MT
	...
	<tr class="even">
		...
		if($modelvoyage->id_bussiness_type_order==250){
			$total = $modelvoyage->total_price ;
		}else{
			$total = $modelvoyage->TCPrice ;
		}
	...
	<tr class="even">
		...
		<td  style="vertical-align:top;text-align:right;">
		<?php //echo($modelvoyage->Capacity * $modelvoyage->Price) * (10/100) ?>
		</td>
		...
		if($modelvoyage->id_bussiness_type_order==250){
			$ppn =  ($modelvoyage->total_price) * (10/100) ;
		}else{
			$ppn =  ($modelvoyage->TCPrice) * (10/100) ;
		}

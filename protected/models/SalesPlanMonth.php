 ...
 *
 * The followings are the available columns in table 'sales_plan':
 * @property string $id_sales_plan
 ...
 */
class SalesPlanMonth extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return SalesPlan the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			...
			array('id_vessel_tug, id_vessel_barge, year, month, id_customer, JettyIdStart, JettyIdEnd, TotalQuantity, QuantityUnit', 'required'),
			array('id_vessel_tug, id_vessel_barge, year, month, voyage_no, JettyIdStart, JettyIdEnd, id_currency', 'numerical', 'integerOnly'=>true),
			array('TotalQuantity, PriceUnit, change_rate, FuelLtr, FuelCost, AgencyCost, DepreciationCost, CrewCost, Rent, SubconCost, IncuranceCost, RepairCost, DockingCost, BrokerageCost, OthersCost, GP_COGM, MarginFuelCost, MarginAgencyCost, MarginDepreciationCost, MarginCrewCost, MarginRent, MarginSubconCost, MarginIncuranceCost, MarginRepairCost, MarginDockingCost, MarginBrokerageCost, MarginOthersCost, SalesAgregatMargin, GP_COGS, TotalStandardCost, TotalSalesCost', 'numerical'),
			array('VoyageName, ip_user_updated', 'length', 'max'=>50),
			array('id_customer, id_fuel_price_unit, FuelPriceUnit', 'length', 'max'=>20),
			array('QuantityUnit', 'length', 'max'=>10),
			array('created_user', 'length', 'max'=>45),
			array('MarginFuelCostMoney, MarginAgencyCostMoney, MarginDepreciationCostMoney, MarginCrewCostMoney, MarginRentMoney, MarginSubconCostMoney, MarginIncuranceCostMoney, MarginRepairCostMoney, MarginDockingCostMoney, MarginBrokerageCostMoney, MarginOthersCostMoney', 'numerical'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_sales_plan, id_vessel_tug, id_vessel_barge, year, month, VoyageName, id_customer, voyage_no, JettyIdStart, JettyIdEnd, TotalQuantity, QuantityUnit, PriceUnit, id_currency, change_rate, FuelLtr, FuelCost, AgencyCost, DepreciationCost, CrewCost, Rent, SubconCost, IncuranceCost, RepairCost, DockingCost, BrokerageCost, OthersCost, GP_COGM, MarginFuelCost, MarginAgencyCost, MarginDepreciationCost, MarginCrewCost, MarginRent, MarginSubconCost, MarginIncuranceCost, MarginRepairCost, MarginDockingCost, MarginBrokerageCost, MarginOthersCost, GP_COGS, created_date, created_user, ip_user_updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'Tug' => array(self::BELONGS_TO, 'Vessel', 'id_vessel_tug'),
			'Barge' => array(self::BELONGS_TO, 'Vessel', 'id_vessel_barge'),
			'Currency' => array(self::BELONGS_TO, 'Currency', 'id_currency'),
			'Customer' => array(self::BELONGS_TO, 'Customer', 'id_customer'),
			'LoadingPort' => array(self::BELONGS_TO, 'Jetty', 'JettyIdStart'),
			'UnloadingPort' => array(self::BELONGS_TO, 'Jetty', 'JettyIdEnd'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'voyage_no' => 'Voyage No',
			'JettyIdStart' => 'Port Loading',
			'JettyIdEnd' => 'Port Unloading',
			...
			'SalesAgregatMargin' => 'Sales Agregat Margin',
			'GP_COGS' => 'GP COGS',
	...
	public function search()
	{
		...
		$criteria->compare('id_sales_plan',$this->id_sales_plan,true);
		...
		$criteria->compare('SalesAgregatMargin',$this->SalesAgregatMargin);
		...
		$criteria->compare('id_fuel_price_unit',$this->id_fuel_price_unit);
		$criteria->compare('FuelPriceUnit',$this->FuelPriceUnit);

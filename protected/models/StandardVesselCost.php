 ...
 */
class StandardVesselCost extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('id_standard_vessel_cost, id_vessel, month, year, depreciation_cost, rent_cost, crew_own_cost, crew_subcont_cost, insurance, repair, docking, brokerage_fee, others, created_date, created_user, ip_user_updated', 'safe', 'on'=>'search'),
		);
	}
	...
	public function cekvesselupdate()
	{
	    ...
	    $model = StandardVesselCost::model()->find(array(
           'condition' => 'id_vessel = :id_vessel  AND id_standard_vessel_cost <>:id_standard_vessel_cost',
           'params' => array(
               ':id_vessel' => $this->id_vessel,
               ':id_standard_vessel_cost' => $this->id_standard_vessel_cost,
           ),
       ));
	    if ($model)
	         $this->addError('id_vessel', 'Vessel sudah Ada di dalam Data Standar Vessel Cost') ;// validasi model gagal sehingga perintah save() akan dibatalkan.

...
<div class="view">	
	...
	<?php
		...
		foreach($listcurrency as $cur){
			if($cur->currency != "IDR"){
				echo '<br>1 '.$cur->currency.'= <input class="span2" maxlength="50" id="'.$cur->currency.'_rate" value="'.$cur->change_rate.'" type="text" disabled>IDR';
...
<?php
	function addRowStandardSales($rowTitle, $model, $STD, $STDfieldname, $form, $fieldColumnStandardDB, $fieldColumnMarginDB, $selectedCurrency){
		...
		$DISP .= '
			...
			<td><div class="selectedcur">'.$selectedCurrency.'</div></td>  
	...
	function addRowStandardSalesFuel($rowTitle, $model, $STD, $STDfieldname, $form, $fieldColumnStandardDB, $fieldColumnMarginDB,$fuelprice, $selectedCurrency){
		...
		$DISP .= '
			...
			<td style="text-align:right;">
				...
				.$form->hiddenField($model,$fieldColumnMarginDB.'Money',array('class'=>'span10','maxLength'=>20,'id'=>$fieldColumnMarginDB.'Count','value'=>$fuel_cost,)).
			'
			</td>
			<td><div class="selectedcur">'.$selectedCurrency.'</div></td> 
		</tr>
...
$(document).ready(function(){
	...
	function calculateGPRevVsStandard(){
		...
		$('#totalRevenue').val(formatcurrencynyaout(total));
